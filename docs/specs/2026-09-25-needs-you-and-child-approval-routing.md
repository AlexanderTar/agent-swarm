...
**agy** is now backed by a live-confirmed row and joins claude, not the cursor exception: refused. **codex** could not be live-confirmed (backend 401, see the row above); Task 9 keeps it refused on source-code confidence per the original decision, since no evidence — live or otherwise — points to the hook not firing. This should be re-verified with a retry once the codex backend issue clears; if a retry ever shows `request_user_input` NOT hooked, codex joins cursor's exception in Task 9 and the refusal must be reverted.
