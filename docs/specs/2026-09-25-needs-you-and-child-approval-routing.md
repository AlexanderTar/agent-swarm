...
| codex | `request_user_input` | **Yes, per source.** `RequestUserInputHandler` does not override `pre_tool_use_payload`/`post_tool_use_payload`. The registry's defaults return `Some(..)` for every `ToolPayload::Function`, so `PreToolUse` and `PostToolUse` run with `tool_name = "request_user_input"`. The docs list "Other local function tools" as hookable and exclude only hosted tools such as `WebSearch`. There is no live run yet. | `{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"…"}}`. The docs also accept the older `{"decision":"block","reason":"…"}`, which the current adapter emits (`internal/adapter/codex.go:139`). | `tool_response` field present; its shape for this tool is unknown | `UserPromptSubmit` (`prompt`) | [Codex hooks docs](https://learn.chatgpt.com/docs/hooks) (was developers.openai.com/codex/hooks), [registry.rs](https://github.com/openai/codex/blob/main/codex-rs/core/src/tools/registry.rs), [request_user_input.rs](https://github.com/openai/codex/blob/main/codex-rs/core/src/tools/handlers/request_user_input.rs), [hook_runtime.rs](https://github.com/openai/codex/blob/main/codex-rs/core/src/hook_runtime.rs) |
...
| agy (Antigravity CLI; its `~/.gemini` config dir is not Gemini CLI) | `ask_question` (also `ask_permission`) | **Yes, per docs.** `PreToolUse` and `PostToolUse` fire for tools matched by name, and the tool list includes "`ask_question`: Ask multiple-choice questions." and "`ask_permission`: Request additional scoped permissions." The current install uses matcher `*` (`internal/install/agy.go:40-43`). There is no live run yet. | `{"decision":"deny","reason":…}`, which the current adapter emits (`internal/adapter/agy.go:182-188`). Values are `allow`, `deny`, `ask`, `force_ask`, `deny_unless_prior_grant`. | **No.** Payload is `toolCall.name`, `toolCall.args`, `stepIdx`, `error`; no result field. | **None.** Events are PreInvocation, PreToolUse, PostToolUse, PostInvocation, Stop. | [Antigravity hooks docs](https://antigravity.google/docs/hooks/), [Where does Antigravity look for Hooks?](https://atamel.dev/posts/2026/07-16_where_agy_hooks/) |
| muse | `request_user_input` | Unknown. The plugin contract has `PreToolUse`; the Task 4 probe will record the payload. | Task 4 | Task 4 | Task 4 | `native-plugin-contract.md` (local) |
...
**What follows for Task 9:** refuse `swarm_ask kind:"question"` for **claude, codex, agy and muse** (muse only once Task 8 lands). **Keep it for cursor**, because its native `AskQuestion` is invisible to Swarm. For cursor agents, `swarm_ask question` stays the only way to reach Needs you, and the cursor skill line says so.
...
5. **Verify.** The daemon requires one question row with `binding_json.ref = ref` and `agent_id = caller agent`, `state = 'answered'`, `responded_via = 'terminal'`, and `responded_at ≥` the ref's creation. It also requires evidence that matches the decision: `response_text`, trimmed and case-folded, must start with the chosen label (`approve` → `Approve`, `request_changes` → `Request changes`).
   ...
   - **Cursor** has no hook, so there is never a row and `native_answer` is refused with `errNoNativeEvidence`.
6. **Record.**
...
7. **Unblock.** `approval_result` is in `ImmediateKinds` (`internal/runtime/inbox.go:17`), so the child or orchestrator is woken at once. Skill rule 6 stays true: "a user approval exists only as an `approval_result` message with a request id."
...
3. Orchestrator answers an approval question by issuing `swarm_ask kind:"native_prompt", for_msg`, then showing the prompt natively, then forwarding with `native_answer` (2.3). It does not also `swarm_send answer`: the daemon's `approval_result` is the answer.
...
- **`skills/swarm/SKILL.md:17`**, from "Top-level agents may use…" onward:
  > "Top-level agents ask the user only with their native question tool (claude `AskUserQuestion`, codex `request_user_input`, agy `ask_question`, muse `request_user_input`); `swarm_ask kind: "question"` is refused. Cursor is the exception: its question tool is invisible to Swarm, so cursor agents use `swarm_ask kind: "question"`. With a parent, send `swarm_send` `kind: "question"` (add `options` when you have choices, and `approval: true` when you need explicit approval), then end your turn. A plain answer arrives as `kind: "answer"` with `reply_to` = your question's `msg_id`; an approval arrives as `approval_result`."
  Delete: "If the user answers you in your terminal, call `swarm_ask` with `withdraw` for any open request."
- **`skills/swarm-orchestrator/SKILL.md:43`** →
  > "- Answering child questions and blockers: for a `kind: "question"` message or a `relay` with `event: "blocked"`, answer from the brief, plan or code if you can. Otherwise ask the user with your native question tool (header `<child> asks`, the child's text and options verbatim) and reply `swarm_send(to: <child>, kind: "answer", reply_to: <msg_id>, body: <the user's choice + note>)`. If the question has `approval: true`, call `swarm_ask kind: "native_prompt", for_msg: <msg_id>`, show the returned `native_prompt` verbatim, then forward with `swarm_ask kind: "native_answer", ref: <msg_id>, decision, comment`; the daemon sends the child `approval_result`. Never use `swarm_ask kind: "question"` for this (cursor excepted). A relay `event: "question_unanswered"` means you still owe that answer."
- **`skills/swarm-orchestrator/SKILL.md:64`** →
  > "- Approving through native question tools: every `swarm_ask` approval (`approval`, `confirm_repos`) returns a `native_prompt`. Show it with your native question tool exactly as given (one per spec section, never batched; plan warnings are already in it). Then forward the user's answer: `swarm_ask kind: "native_answer", ref: <request_id>, decision: "approve" | "request_changes", comment`. The daemon records the approval only if it saw that prompt answered in your terminal with the same choice; it then sends `approval_result`. Never forward a decision the user did not pick. Only top-level orchestrators, interactive TUI only. Cursor has no native path: the user approves in the board or with `swarm approve`."
...
None. Every question from revs 1 and 2 is answered (sections 1.5 and 1.6). The only pending facts come from the live probes: Task 4 (muse) and Task 4b (codex and agy). Their outcome is already decided: a kind that fails joins cursor's exception in Task 9, and a kind with no answer text records `agent_reported`.
