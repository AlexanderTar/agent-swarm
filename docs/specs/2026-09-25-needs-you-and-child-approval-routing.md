...
6. **Skill rule 6 has a uniform exception (2026-09-26, final review finding 1, widened same day).** A plain `answer` from a child's own parent, `reply_to` the child's own `approval: true` question, counts as the user's approval decision — for **any** parent agent kind, not only kinds without a native approval hook. First drafted as a per-kind carve-out (unhooked parents can never produce a bound, hook-observed `approval_result`, so refusing to define an answer path there leaves the child waiting forever), the user then widened it to every kind: relaying the decision through the parent is uniform and simpler than a per-kind rule, and the user accepts the weaker trust model this implies for child approvals specifically. `approval_result` via observed native evidence is unaffected and still applies to every request the daemon itself owns (`approve_section`/`approve_plan`/`approve_report`, `confirm_repos`, `close_spike`) — only a child's own approval question (a msg_ ref, §2.4) gets this fallback, and native evidence stays available there too for a parent kind whose question tool is hooked. `skills/swarm/SKILL.md` (the `:17` bullet and rule 6), `skills/swarm-orchestrator/SKILL.md:43`, and `errChildApprovalNoNativePath`'s comment (`internal/runtime/native.go`) all state this without naming specific kinds. The routing preconditions — the answer names the child's own question, and the answering agent really is that child's parent — are already enforced by `Send`'s `reply_to` validation (`internal/runtime/inbox.go:488-556`, task T10; see its added comment); no new code guard was needed.
...
1. **D1: typed free text is accepted on any daemon-issued native approval prompt.** The orchestrator interprets the user's typed ("Other") answer to a `native_prompt`, and forwards `decision: "approve" | "request_changes"` with that text as `comment`; the daemon does not refuse it as a mismatch. `native_answer` records it with `approval_evidence = agent_reported` (the typed text is the evidence — it becomes the stored `comment` when the caller sends none). Rationale: keep it simple, not too tight — the daemon cannot itself interpret free text, so it trusts the orchestrator's forwarded decision the same way it already trusts an agent-reported "Resolved in terminal" answer. `matchDecisionEvidence` (`internal/runtime/native.go`) still refuses a **mismatch**: text that (case-fold) IS the *other* decision's option label, or that label followed by `":"` — the user visibly picked "Approve" but the orchestrator forwarded `request_changes`, or the reverse — because that is forgeable evidence of the opposite decision, not free text. (REVISED 2026-09-26, same day, finding B6-2: narrowed from a bare "starts with" prefix, which wrongly refused typed free text like "Approve, but drop endurio-docs" forwarded as `request_changes`, and wrongly marked the same text `observed` when forwarded as `approve`.) Text that merely starts with a label's letters without that exact shape is agent_reported, not a mismatch and not observed. The "plain Request changes with no comment is refused" rule (`nativeAnswer`'s comment-required check) is removed on the native path only: a comment is optional for `request_changes` through `native_answer`; the 2000-character cap stays. Board/CLI `RequestChanges` (`internal/runtime/requests.go`) is untouched and still requires a comment there — this decision only relaxes the native, typed-text path. Typed free text also survives on an Approve decision: `nativeAnswer` stores it as the request's own `response_text`, the same way `request_changes` and `confirm_repos` already do.
...
**Trust model (stated plainly):** agents run as the user's UID and can read every token (F12), so no design can stop a deliberately malicious agent. The threat this path guards against is an agent *claiming* an approval the user never gave (hallucination, or a misread free-text answer). Rule: **an approval is recorded only if the daemon observed the user answering a daemon-issued prompt in a hook event, and the forwarded decision matches the observed answer.** The MCP call forwards; the hook evidence decides. (REVISED 2026-09-26 (B6 §1.8): "matches" means the exact-or-`"label:"` shape in §1.8 D1/§2.3.5, not a bare prefix — typed free text that merely starts with a label's letters (`"Approve, but drop endurio-docs"`) is neither a match nor a mismatch; it is agent_reported.)

**Accepted exception (2026-09-26, user decision, §1.6):** for a child's own approval question specifically (a msg_ ref, §2.4), this rule has one uniform exception that applies to **any** parent agent kind: the parent's own `swarm_send kind:"answer"` to the child's `approval: true` question, matched by `reply_to`, counts as the approval, on the parent's word alone, with no hook evidence and no `evidence` audit field. This was first scoped to parent kinds with no native approval hook at all — there is no hook event to observe, ever, for those, and refusing to define an answer path there leaves the child waiting forever (final review finding 1) — then widened the same day to every parent kind: the user accepts the weaker trust model for child approvals, and relaying the decision through the parent uniformly is simpler than a rule that branches on kind. This is narrower than the existing agent-reported path (step 5): it has no bound question row at all, because there is nothing to bind it — the daemon's `Send` validation (`internal/runtime/inbox.go:488-556`, `T10`) already guarantees the answer's `reply_to` is that exact child's own `question` addressed to the answering agent, so the routing (right child, right parent) is enforced; only the "was this really the user" trust is weaker. Scope: only child approval questions (`for_msg`/message-ref, §2.4). Every request the daemon itself owns (`approve_section`/`approve_plan`/`approve_report`, `confirm_repos`, `close_spike`) keeps the full hook-evidence rule unconditionally — this exception never applies there.
...
3. **Observe.** At PreToolUse, `extractQuestion` finds the ref token. `AskQuestion` then opens the question row with `binding_json = {"ref":"<ref>"}` (a new optional argument; no schema change, since `binding_json` exists). At PostToolUse, `ResolveQuestionByPrompt` sets the row to `answered` via `terminal`, with `response_text` = the tool response text.
   ...
   **2026-09-26 addendum (native-railway-tracing finding).** A live spike issued 10 section approvals as 10 separate `AskUserQuestion` calls, and the hook bound and answered all 10 — but the agent never called step 4's `native_answer`, because it had loaded a `swarm-orchestrator` skill copy from before `native_answer` existed and nothing else said there was a next step. Two fixes close that gap, both required together:
   - `requestOut` (step 1) now returns a `next` field alongside `native_prompt` whenever `native_prompt` is set: the same "show it, then forward it" instruction, so a stale skill still gets told at the point of issue.
   - PostToolUse's hook output (step 3, `additionalContext`/`ephemeralMessage` per adapter) now says, once the bound row is `answered`: `[swarm] Recorded "<label>" for <ref>. Forward it now: swarm_ask kind:"native_answer", ref:"<ref>", decision:"<approve|request_changes>"` — or, when the adapter has no response text (agent-reported), `…decide approve or request_changes from the user's text "<text>"`. This is never rate-limited by the pending-notice gap: a missed forward strands the user's actual answer, unlike an informational notice.
   - A batched `AskUserQuestion` call is refused at PreToolUse when any of its questions carries a `⟦swarm:ref⟧` token, since the hook only ever binds `Questions[0]` (`handler.go:~49`) and every later ref in the same call would otherwise be silently unbindable: `"[swarm] Ask one swarm approval per question call."`
   - `truncateWithToken` was already correct (truncates the body, keeps the trailing ref token); a regression test was added since none existed.
   - `skills/swarm-orchestrator/SKILL.md` (native approval rule, and the child-approval sentence) now states the full loop explicitly: print the section's summary in chat (never in the question), show `native_prompt` verbatim with no added text, one question per call, then call `native_answer` immediately in the same turn.
4. **Forward (MCP).** The orchestrator calls `swarm_ask {kind:"native_answer", ref, decision:"approve"|"request_changes", comment?}`.
5. **Verify.** The daemon requires one question row with `binding_json.ref = ref` and `agent_id = caller agent`, `state = 'answered'`, `responded_via = 'terminal'`, and `responded_at ≥` the ref's creation. It also requires evidence that matches the decision: `response_text`, trimmed and case-folded, must equal the chosen label, or that label followed by `":"` (`approve` → `Approve`, `request_changes` → `Request changes`) — see the REVISED note below.
   - **Agent-reported (locked, 1.6.2).** Some adapters have PostToolUse but no response text: agy per 1.7, and muse if Task 4 finds the same. The bound row then shows `"Resolved in terminal"`, so the daemon has proof the prompt was answered but not of which option. It accepts the forwarded decision **on the agent's word** and records `evidence = "agent_reported"`. When the text matches the label, it records `evidence = "observed"`. When the text is present but does not match, the call is still refused (`errDecisionMismatch`). (REVISED 2026-09-26 (B6 §1.8): refusal applies only when the text IS the *other* decision's label, or that label followed by `":"` — a picked-and-contradicting option. Any other typed text, including text that starts with a label's letters without that exact shape, is agent_reported, never refused.)
   - **A parent kind with no native question hook** has no row to observe. For a request-ref (step 1's first bullet) `native_answer` is refused with `errNoNativeEvidence`, and the board/`swarm approve` fallback (native_pending) is unaffected. For a message-ref (a child's approval, step 1's second bullet) there is no such fallback, so `native_prompt`/`native_answer` refuse up front with `errChildApprovalNoNativePath` — see 2.4 step 3. (REVISED 2026-09-26, finding 1, widened same day: the child does not go unanswered even so, because a plain `answer` from its parent is now always a valid approval decision for any parent kind, not only one without the hook — see the accepted exception above and §1.6.)
6. **Record.**
...
7. **Unblock.** `approval_result` is in `ImmediateKinds` (`internal/runtime/inbox.go:17`), so the child or orchestrator is woken at once. Skill rule 6 stays true — "a user approval exists only as an `approval_result` message with a request id" — **with one exception, accepted by the user on 2026-09-26 (final review finding 1, widened same day, see §1.6):** for a child's own approval question, its parent can instead answer with a plain `swarm_send kind:"answer"` (matched by `reply_to` to that question), and the child treats it as the approval decision. This applies to any parent agent kind, not only kinds without a native approval hook — a uniform rule is simpler than branching on kind, and the user accepts the weaker trust model this implies for child approvals specifically; every request the daemon itself owns keeps the full hook-evidence rule. `skills/swarm/SKILL.md` rule 6 and the `:17` bullet both carry this exception; `errChildApprovalNoNativePath`'s comment (`internal/runtime/native.go`) records the same decision for the kinds where native evidence still refuses.
...
3. Orchestrator answers an approval question by issuing `swarm_ask kind:"native_prompt", for_msg`, then showing the prompt natively, then forwarding with `native_answer` (2.3), when its agent kind has a native approval hook. When it does not, both calls refuse (`errChildApprovalNoNativePath`) instead of building a prompt that can never be answered — the hook that would bind evidence to it never fires. (REVISED 2026-09-26, finding 1, widened same day.) Either way — hook or not — the orchestrator may instead skip 2.3 entirely and `swarm_send {to:<child>, kind:"answer", reply_to:<msg_id>, body}` the same as a plain question; the child treats that plain answer as its approval decision, for any parent kind. It does not also send both: an `approval_result` (native path) or a plain `answer` (this fallback) is the answer, not one after the other.
...
- **`skills/swarm-orchestrator/SKILL.md:64`** → (REVISED 2026-09-26: same fix, request-ref side. This path already had a working fallback — the board/`swarm approve` resolve the request row directly, no native_answer needed — so only the wording changes. REVISED AGAIN 2026-09-26, B6 §1.8: typed ("Other") answers are forwarded with whichever decision the user meant, and confirm_repos' Approve now confirms proposed + expansion together.)
