...
not "session-4f2a" or "Codex task"). Summary: 2-5 sentences covering goal, current
state, next step. Tags: lowercase filter labels, e.g. `["agent-swarm", "daemon", "bugfix"]`.

If a session briefing gave you a swarm session id, pass it as `sessionId`. Otherwise the
daemon binds your MCP calls to the session running in this working directory — you do not
need to pass `sessionId`.
