...
You do, in your own words, through MCP server `swarm`.
...
- `swarm_board` / `swarm_task_get` to see what other agents are on.
...
not "session-4f2a"). Summary: 2-5 sentences covering goal, current state, next step.
Tags: lowercase filter labels, e.g. `["agent-swarm", "daemon", "bugfix"]`.
