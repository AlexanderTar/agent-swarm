-- 0004_hitl_requests.sql: add 'prompt' and 'blocker' to requests.kind, add 'is_hitl' column
...
SELECT
  id, kind, CASE WHEN kind = 'question' THEN 1 ELSE 0 END,
  agent_id, session_id, item_id, artifact_id,
  section_id, section_sha256, prompt, options_json, state,
  confirmed_json, artifact_revision, binding_json, response_text,
  responded_via, responded_at, created_at
FROM requests;
