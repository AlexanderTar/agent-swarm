...
// full trace and why it must be the "daily" host, not "prod"). Muse spends
// one zero-effort MSP turn (Muse.Fetch, muse.go) on the normal cadence — no
// Keychain access at all, see muse.go's doc comment for why.
// Nothing here is called by any test in this package — the only thing
// exercised is that the slice has five entries (S-4).
//
// 2026-09-24: a brief keychain+mint-API replacement for Muse
// (docs/specs/2026-09-23-muse-keychain-api-usage.md) was reverted the same
// day it shipped: `muse login`/token-refresh rewrites its own Keychain item
// and resets the item's ACL every time, silently undoing any one-time
// `security set-generic-password-partition-list` widen, so the Keychain
