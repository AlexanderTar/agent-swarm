...
//
// This is a 2026-09-24 revert of the brief keychain+mint-API replacement
// (museapi.go, docs/specs/2026-09-23-muse-keychain-api-usage.md): that
// source's `ai.meta.dev.credentials` Keychain item is ACL'd to Muse's own
// signing team, and `muse login`/token-refresh rewrites the item and resets
// that ACL every time it runs -- any one-time `security
// set-generic-password-partition-list` widen gets silently undone, so the
// prompt comes back on an unpredictable cadence tied to Muse's own refresh,
// not swarm's. This probe touches the Keychain not at all.
//
